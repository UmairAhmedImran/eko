...
func resolveCleanTargets(candidates []cleanCandidate) ([]cleanCandidate, error) {
	...
	for _, c := range candidates {
		...
		// EvalSymlinks resolves every link in the path and fails when the
		// directory is missing, so a stale row cannot drive a deletion.
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		for i, t := range targets {
			...
			if _, err := database.Exec("DELETE FROM snapshots WHERE id = ?", t.id); err != nil {
				return fmt.Errorf("removed %d of %d snapshot(s); removed %s from disk but could not delete its database row: %w", i, len(targets), t.path, err)
			}
